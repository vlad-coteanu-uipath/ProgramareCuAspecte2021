	...
	public void paint(Graphics g) {
		...
		if (total != 0) {
			...
			for (int i = 0; i < sliders.size(); i++) {
				...
				//draw the arc
				g.setColor(LayoutConstants.courseColours[i]);
				g.fillArc(LayoutConstants.xOffset, LayoutConstants.yOffset + 250, 2 * radius, 2 * radius, (int) startAngle, (int) ratio);
				
				DecimalFormat df = new DecimalFormat("####0.00");
				String percent = df.format((record.getValue() / total) * 100) + "%";
				
				g.drawString(record.getName() + " " + percent, LayoutConstants.xOffset - 100, LayoutConstants.yOffset + 350 + i * 25);
				startAngle += ratio;
			}
		}
	}
